...
- **Host playback from a desktop browser.** iOS Safari only lets one video play sound at a time — the outgoing song stops the moment the next one starts (so no real crossfade), and the very first automatic transition needs a one-time manual tap before iOS allows programmatic playback. Playback still works on an iPhone/iPad, just degraded; a laptop or desktop connected to the speakers is the intended setup. Guests' phones are unaffected — they never play audio. See `docs/decisions.md` #41.
