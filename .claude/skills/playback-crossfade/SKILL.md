...
- Autoplay: the first user-initiated "Play" click provides the page's user-gesture allowance; subsequent programmatic `playVideo()` calls on the standby player should keep working within that session, but this is the riskiest unknown in this design, especially on mobile Safari. Spike/test this on real devices before building the rest of playback.
