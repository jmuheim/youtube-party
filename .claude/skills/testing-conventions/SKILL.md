...
**Automated accessibility checks run inside system specs.** Use the `axe-cuprite` gem so every page a system spec visits gets a WCAG audit: `expect(page).to be_axe_clean` (scoped/excluded where third-party content like the YouTube iframes can't be fixed by us). Add the assertion to the main happy-path spec of each feature area rather than a separate a11y suite — it rides along for free on pages the specs already visit. This automates the *detectable* part of the accessibility requirement (missing labels, contrast, ARIA misuse); keyboard-flow and screen-reader behavior still need the manual checks from the `accessibility` skill — axe passing is necessary, not sufficient. Note: we use `axe-cuprite` not Deque's `axe-core-rspec` — the latter calls Selenium-specific driver internals (`browser.manage`) that don't exist in Ferrum; `axe-cuprite` drives axe entirely through Capybara's driver-neutral JS API. See decision #39.
