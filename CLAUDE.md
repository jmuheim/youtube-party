...
**Implementation status (roadmap PR 0.2, crossfade spike):** the core two-player crossfade exists at `/playback` — `app/javascript/controllers/player_controller.js` (slot management, 250ms poll, fade loop with an injectable scheduler per decision #24) plus `app/javascript/crossfade_math.js` (pure scheduling functions). Videos are hardcoded in `PlaybackController::SPIKE_VIDEOS` until real models land (PR 1.1); beat classification, transition sounds, skip segments, cancellation architecture, wake lock, and error auto-skip are not built yet (later PRs). JS testing approach: decision #40. Real-device spike findings: decision #41 — desktop hosts work as designed; iOS Safari can't overlap two unmuted videos, so crossfades degrade to hard cuts there and the host should be a desktop browser.
